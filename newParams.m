...
for p = L*N+1:2:L*N+NK-1
    cylCoord(p, :) = [cvxi, cvy, aD];
    cylCoord(p+1, :) = [cvxi, -cvy, aD];
...
ystart = min(cylCoord(:, 2))+28.5*dy;
yfinal = max(cylCoord(:, 2))-28.5*dy;
rangey = abs(ystart)+abs(yfinal);
rangex = rangey*2;
xstart = min(cylCoord(:, 1))+dx;
x = linspace(xstart, xstart+rangex, 220);
y = linspace(ystart, yfinal, 110);
