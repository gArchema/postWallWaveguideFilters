...
%find A matrix
C = zeros(L*N*(2*M+1)+D*(2*M+1), L*N*(2*M+1)+D*(2*M+1));
F = zeros(L*N*(2*M+1)+D*(2*M+1), 1);
for p=1:L*N+D
...
%     end;
    ...
    for q=1:L*N+D
        ...
        for m = -M:M
            for n = -M:M
                if p~=q
                    c = -besselh(m-n, 2, k1*dc)*exp((-1i)*(m-n)*fc);
                elseif m==n
%                     syms xx real;
%                     beshd = diff(besselj(m,xx) - 1i*bessely(m, xx));
%                     beshd = subs(beshd, xx, k1*cylCoord(p, 3));
...
x = linspace(cylCoord(1, 1)-2*dx, cylCoord(N+D, 1)+3*dx, 100);
...
%             end;
            ...
            for p=1:L*N+D
                ...
                for m = -M:M
                    ...
                    esctmp = esctmp + (1i)^(m)*besselj(m, k*des)*exp(1i*m*(fes-fii));
                end;
