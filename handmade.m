...
cxh = xs + 2*aP+aD;
cyh = (LL*NK/2+1)*dyv;
for p = L*N+LL*NK+D+1:2:L*N+LL*NK+D+Nh-1
...
%find A matrix
C = zeros((L*N+D+LL*NK+Nh)*(2*M+1), (L*N+D+LL*NK+Nh)*(2*M+1));
F = zeros((L*N+D+LL*NK+Nh)*(2*M+1), 1);
T = zeros((L*N+D+LL*NK+Nh)*(2*M+1), 1);
for p=1:L*N+D+LL*NK+Nh
...
ystart = min(cylCoord(:, 2))-2*dy; %5.8*wg;
yfinal = max(cylCoord(:, 2))+2*dy; %5.8*wg;
...
coef = normpdf(y,0,wg/2+(L-1)*dy/2);
K = coef(floor((size(coef,2))/2)+1);
coef = coef/K;
...
for yp = 1:size(y, 2)
    for xp = 1:size(x, 2)
        if cylContainsXY(cylCoord, x(xp), y(yp))==-1% && (y(yp)<wg/2 && y(yp)>-wg/2)
            ...
            ei =  coef(yp)*exp(1i*kE*dei*cos(fei-fii));
            es = 0;
            eesc = 0;
            for p=1:L*N+D+LL*NK+Nh
